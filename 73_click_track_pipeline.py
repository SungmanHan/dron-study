...
이 기계에서는 분할(floodFill) 1 ms · 추적 7~60 ms(KCF~MIL) 였다 — 즉 여기서는 분할이 더 싸다.
이 구조가 이득인 건 **분할이 SAM 처럼 비쌀 때**(초 단위) 뿐이다. 근거는 72 참고.

[강의와 다르게 만든 것]

1. **CSRT 를 고정해 쓰지 않는다.** CSRT 는 `opencv-contrib-python` 에만 있어서
   일반 `opencv-python` 환경에서는 `cv2.legacy` 대비책까지 함께 없다(72 에서 확인).
   `create_tracker()` 가 CSRT -> KCF -> MIL 순으로 **있는 것**을 쓴다.
2. **`success` 를 믿지 않는다.** 가림 실험에서 CSRT·KCF 는 놓친 뒤 `False` 를 69 회 돌려줬지만
   **MIL 은 0 회**였다 — 박스가 멈춘 채 계속 "성공" 이다(72). 폴백으로 MIL 이 잡히는 환경에서는
   `success` 만으로 해제를 판단할 수 없다. 그래서 첫 박스의 색 분포와 현재 박스를 비교해
   `LOST_TH` 미만이 `LOST_K` 프레임 연속이면 놓친 것으로 본다.
