...
넘어가라고 안내한다. 이 저장소의 `opencv-python` 4.10.0 에서 실제로 확인해 보면 **둘 다 없다**.

| 호출 | 결과 |
|---|---|
| `cv.TrackerMIL_create()` | **OK** (가중치 파일 불필요) |
| `cv.TrackerGOTURN_create()` | 실패 — caffe 모델 파일 필요 |
| `cv.TrackerDaSiamRPN_create()` / `Nano` / `Vit` | 실패 — onnx 모델 파일 필요 |
| `cv.TrackerCSRT_create` | **없음** (contrib 전용) |
| `cv.legacy.TrackerCSRT_create()` | `AttributeError: module 'cv2' has no attribute 'legacy'` |

`legacy` 모듈 자체가 contrib 쪽이라 **대비책이 대비가 되지 않는다**.
CSRT·KCF·MOSSE 를 쓰려면 패키지를 갈아야 한다(둘은 같은 `cv2` 폴더에 설치돼 함께 두면 충돌한다).

```bash
pip uninstall -y opencv-python
pip install "opencv-contrib-python==4.10.0.84"   # macOS 12 Intel 용 휠 있음 (66MB)
```

이 저장소는 나머지 강의와의 호환을 위해 일반 `opencv-python` 을 유지하고,
`create_tracker()` 가 **CSRT → KCF → MIL** 순으로 있는 것을 골라 쓴다.

### "검출은 무겁고 추적은 가볍다" 는 절반만 맞다

같은 합성 영상 120 프레임으로 잰 값이다.

| | 속도 | IoU 평균 (가림 없음) | IoU 평균 (가림 있음) |
|---|---|---|---|
| 추적 `MIL.update()` | **61.7 ms/frame (16 fps)** | 0.948 | 0.425 (최종 0.000) |
| 재검출 (HSV 색 검출) | **1.5 ms/frame (649 fps)** | 0.951 | 0.817 (최종 0.967) |

(MIL 은 내부에 무작위 표본을 써서 실행마다 수치가 조금씩 흔들린다.)

추적이 재검출보다 **40배 느리다**. 강의의 비교 대상이 SAM 이라서 맞는 말이었을 뿐이다
(같은 기계에서 grabCut 분할은 **758 ms**, floodFill 은 1.0 ms).
**가벼운 검출기가 있으면 추적기를 끼워 넣을 이유가 없다.** 추적은 속도 수단이 아니라
**비싼 검출기를 덜 부르는** 수단이다.

가림 실험의 차이가 더 중요하다. 추적기는 기둥 뒤로 사라진 물체를 **끝까지 되찾지 못했고**,
매 프레임 새로 찾는 검출은 기둥을 지나자마자 복귀했다. 추적은 상태를 이어 가는 대신
**한 번 틀리면 계속 틀린다**.

### ⛔ `update()` 는 실패를 알려주지 않는다

가림 실험에서 **119 프레임 내내 `success == True`** 였다. 물체를 완전히 놓친 뒤에도
박스는 `(263, 105)` 에 멈춘 채 계속 "성공" 을 돌려줬다(IoU 0.000).
강의 코드는 `success` 만 보고 추적 해제를 판단하므로, 이 상태에서
**엉뚱한 좌표를 드론 제어에 계속 흘려보낸다.**

그래서 `73` 은 외부 확인을 하나 붙였다 — 첫 박스의 **색 히스토그램**과 현재 박스를 비교해
유사도가 `LOST_TH`(0.5) 미만으로 `LOST_K`(5) 프레임 연속이면 놓친 것으로 본다.

5 회 실행한 값이다.

| | 선언 시점 |
|---|---|
| `success == False` | **영영 안 옴** (5 회 모두 0 회) |
| 히스토그램 가드 | 55, 55, 55, 55, 55 프레임 |
| (참고) 실제 IoU 붕괴 | 59, 54, 55, 61, 55 프레임 |

`success` 가 끝내 알려주지 않는 시점을 가드는 **붕괴와 같은 프레임에** 잡아낸다.
한 프레임만 보면 0.819 처럼 튀는 값이 나오므로 **연속 K 회** 조건이 필요하다.
가림 없는 영상에서는 5 회 모두 한 번도 선언하지 않았다(오탐 0).
...
`update` 가 60 ms 대라 실제 루프는 `clock.tick(30)` 을 못 채우고 16 fps 로 돈다 —
빠르게 움직이는 물체에서는 박스가 눈에 띄게 뒤처진다.
...
| `AttributeError: TrackerCSRT_create` | CSRT 는 `opencv-contrib-python` 에만 있다 | contrib 로 교체하거나 MIL 로 대체 (`create_tracker()`) |
...
| 물체를 놓쳤는데 박스가 그대로 남음 | `update()` 가 놓친 뒤에도 `success=True` (실측 119/119) | 색 히스토그램 유사도로 별도 판정 |
...
| 30 FPS 로 설정했는데 16 fps | `MIL.update()` 자체가 60 ms 대 | 정상. 추적기를 바꾸거나 해상도를 낮춘다 |
