...
| `71_mask_to_bbox.py` | 마스크 → 바운딩 박스 — 윤곽선·중심점·화면 중앙 오차 📷 |
| `72_tracker_backend.py` | 추적기 고르기 — 이 환경엔 CSRT 가 없다 / 추적 vs 재검출 실측 |
| `73_click_track_pipeline.py` | 클릭 → 분할 → bbox → 추적 — 놓침 판정까지 📷 |
| `74_ar_game_physics.py` | AR 게임의 숫자 — 점수·유도·충돌을 실행 전에 검증 |
| `75_drone_ar_game.py` | [과제] Drone AR Game — 미사일 피하기 📷 |
...
## 마스크를 네 숫자로 — 추적·제어가 쓰기 좋은 형태

34 강. (`71`) 드론이 물체를 따라가려면 픽셀 마스크 전체가 아니라
**"화면 중앙에서 얼마나 벗어났는가"** 만 있으면 된다. 그래서 마스크를 `(x, y, w, h)` 로 줄인다.
...
SAM 마스크에는 조각이 섞이므로 **가장 큰 덩어리**를 대상으로 본다.
...
그만큼 어긋난다 — `71` 이 bbox 중심(빨강)과 **마스크 무게중심(노랑)** 을 함께 찍는 이유다.

### 걸러야 할 것 둘

- **작은 조각**: `MIN_AREA` 보다 작은 윤곽선은 버린다. 6x6 조각만 있을 때
  `min_area=100` → `None`, `min_area=10` → `(10, 10, 6, 6)`.
- **크기 불일치**: 모델이 다른 해상도로 마스크를 주면 좌표가 그 해상도 기준이다.
  절반 크기 마스크의 bbox 는 `(101, 71, 99, 99)` — 원본 `(201, 141, 199, 199)` 의 절반이다.
  `cv.resize(..., **INTER_NEAREST**)` 로 프레임 크기에 맞추면 ±1 픽셀 안에서 복원된다
  (마스크는 0/1 이라 보간하면 안 된다).

SAM 이 한 프롬프트에 여러 후보를 내므로, 무조건 `[0]` 이 아니라 **클릭한 점을 포함하는 마스크**를 고른다.

### 강의 원본 코드의 문제

- `sam_processing = None` — `not None` 이 True 라 **우연히** 동작할 뿐이다. 의도는 `False`.
- `get_bbox_from_mask` 가 윤곽선이 없을 때 반환을 명시하지 않는다(암묵적 `None`).
- 카메라 해상도가 640x480 이 아니면 **클릭 좌표와 박스가 어긋난다** → `cv.resize` 로 고정.
- 예외가 나면 카메라가 잡힌 채 남는다 → `try/finally`. 박스를 지울 방법도 없다 → 오른쪽 클릭 / `C`.

### 다음 단계

SAM 은 무거워 매 프레임 돌릴 수 없다. 실제 추적은 **SAM 으로 첫 bbox 만 만들고
가벼운 추적기로 그 박스를 따라가는** 구조가 보통이다 — 35 강(`72`, `73`)에서 만든다.
`71` 이 찍는 `offset=(dx, dy)` 가 그다음 단계에서 드론을 돌릴 입력이 된다.

## 클릭 한 번으로 따라가기 — 그리고 추적기가 말하지 않는 것

35 강. (`72`, `73`) 구조는 단순하다. **클릭 → 분할 → bbox → 추적기 init**, 그다음부터는
매 프레임 `tracker.update()` 로 박스만 갱신한다. 무거운 분할은 처음 한 번뿐이다.
...
### CSRT 는 contrib 에만 있다

강의는 `cv2.TrackerCSRT_create()` 를 쓰고, 없으면 `cv2.legacy.TrackerCSRT_create()` 로
넘어가라고 안내한다. 일반 `opencv-python` 4.10.0.84 에서 확인하면 **둘 다 없다**
(바이너리에 `TrackerCSRT` 심볼 0 개, `cv.legacy` 는 `AttributeError`).
`legacy` 모듈 자체가 contrib 쪽이라 **대비책이 대비가 되지 않는다.**

```bash
pip uninstall -y opencv-python                    # 같은 cv2 폴더라 함께 두면 충돌
pip install "opencv-contrib-python==4.10.0.84"    # macOS 12 Intel 휠 있음 (66MB)
```

`create_tracker()` 는 **CSRT → KCF → MIL** 순으로 있는 것을 고른다. 가중치 파일이 필요한
GOTURN·Vit·Nano·DaSiamRPN 은 건너뛴다(`onnx`/`caffe` 파일이 없으면 생성 자체가 실패한다).

### 세 추적기와 재검출 — 같은 영상 120 프레임

contrib 로 교체한 환경에서 잰 값이다.

| | 속도 | IoU 평균 (가림 없음) | IoU 평균 (가림 있음) | 놓쳤을 때 `success=False` |
|---|---|---|---|---|
| CSRT | 47.9 ms (21 fps) | 0.933 | 0.396 → 최종 0.000 | 69 회 |
| KCF | **7.0 ms (144 fps)** | 0.822 | 0.346 → 최종 0.000 | 69 회 |
| MIL | 56.9 ms (18 fps) | 0.773 | 0.428 → 최종 0.000 | **0 회** ⚠️ |
| HSV 재검출 | **1.2 ms (861 fps)** | 0.951 | 0.817 → 최종 **0.967** | — |

(MIL 은 내부에 무작위 표본을 써서 실행마다 수치가 조금씩 흔들린다.)

셋 다 **HSV 재검출보다 느리다.** 강의의 비교 대상이 SAM 이라서 맞는 말이었을 뿐이다
(같은 기계에서 grabCut 분할 **758 ms**, floodFill 1.0 ms).
가벼운 검출기가 있으면 추적기를 끼워 넣을 이유가 없다 — 추적은 속도 수단이 아니라
**비싼 검출기를 덜 부르는** 수단이다.

가림 실험의 차이가 더 중요하다. 추적기는 기둥 뒤로 사라진 물체를 **셋 다 되찾지 못했고**,
매 프레임 새로 찾는 검출은 기둥을 지나자마자 복귀했다. 추적은 상태를 이어 가는 대신
**한 번 틀리면 계속 틀린다.**

### ⛔ MIL 은 놓친 것을 알려주지 않는다

CSRT·KCF 는 놓친 뒤 `success = False` 를 69 회 돌려줬지만 **MIL 은 0 회**였다.
박스는 멈춘 채 계속 "성공" 이다. contrib 가 없어 MIL 로 폴백하는 환경에서
`success` 만 보고 해제를 판단하면 **엉뚱한 좌표를 드론 제어에 계속 흘려보낸다.**

그래서 `73` 은 추적기와 무관한 외부 확인을 붙였다 — 첫 박스의 **색 히스토그램**과 현재 박스를
비교해 유사도가 `LOST_TH`(0.5) 미만으로 `LOST_K`(5) 프레임 연속이면 놓친 것으로 본다.

| | 선언 시점 |
|---|---|
| 히스토그램 가드 | 55 프레임 (추적기와 무관하게 같은 자리) |
| 실제 IoU 붕괴 | CSRT·KCF 51, MIL 59 프레임 |
| `success == False` | CSRT·KCF 51 프레임부터 / **MIL 은 없음** |

한 프레임만 보면 0.819 처럼 튀는 값이 나오므로 **연속 K 회** 조건이 필요하다.
가림 없는 영상에서는 한 번도 선언하지 않았다(오탐 0).

### 강의 원본 코드에서 더 고친 것

- **클릭해도 이전 tracker 가 살아 있다** → 분할이 실패하면 옛 박스가 되살아난다. 클릭 즉시 해제.
- **너무 작은 박스로 `init`** → `MIN_BOX`(10px) 미만이면 초기화하지 않는다.
  배경을 잘못 클릭하면 몇 픽셀짜리 박스가 잡히고, 그걸로 학습한 추적기는 곧바로 흘러간다.
- `sam_processing = None` → `False`, 마스크 해상도 보정, `try/finally` — 34 강과 같다.

### 다음 단계

`73` 이 화면에 찍는 `offset=(dx, dy)` 가 그대로 드론 제어 입력이 된다.
좌우 오차는 yaw, 상하 오차는 throttle, 박스 크기 변화는 pitch 로 바꾼다.
비례 게인·최대값 제한·데드존이 기본이고, **비상 정지는 항상 준비해 둔다**.
MIL 은 `update` 가 60 ms 대라 실제 루프가 `clock.tick(30)` 을 못 채우고 16 fps 로 돈다 —
빠르게 움직이는 물체에서는 박스가 눈에 띄게 뒤처진다(CSRT 는 28 fps, KCF 는 여유가 있다).

## 마지막 과제 — AR 게임으로 끝내기

36 강. (`74`, `75`) 웹캠에 비친 드론을 고르면 추적기가 따라가고, 화면 가장자리에서
유도 미사일이 날아온다. **버틴 시간이 곧 점수**다. 지금까지 만든 것이 전부 들어간다 —
분할(33~34강) → bbox(34강) → 추적(35강) → pygame 출력(28강) → 드론 LED(17~20강).

```
SELECT ──(클릭/드래그)──> READY ──SPACE──> PLAY ──(피격)──> OVER ──R──> READY
                                             │ ▲
                                    (추적 놓침) ▼ │ (재선택 후 SPACE)
                                           PAUSED
```

### 점수는 `clock.tick()` 누적 — 다만 물리와 같은 값을 써야 한다

`clock.tick(fps)` 은 **FPS 제한과 경과 시간 측정을 겸한다.** 125 ms 짜리 작업을 끼워 넣고
불러 보면 그대로 `125` 가 나온다. 그래서 느린 PC 에서도 점수는 실시간(초)과 같다.

문제는 원본이 **점수에는 자르지 않은 `delta_tick`, 물리에는 `min(dt, 0.1)`** 을 쓴다는 점이다.

| fps | 점수 가산 | 물리 dt | 세계 진행 |
|---|---|---|---|
| 30 | 33.3 ms | 33.3 ms | 100% |
| 10 | 100.0 ms | 100.0 ms | 100% |
| 8 | 125.0 ms | 100.0 ms | **80%** |
| 5 | 200.0 ms | 100.0 ms | **50%** |

10 fps 밑에서는 **세계가 느려지는데 점수는 그대로 올라 느린 PC 가 유리해진다.**
강의 안내 자체가 "Intel Mac 은 느리다" 이므로 그냥 넘길 문제가 아니다 → `75` 는 둘 다 `dt` 로 맞췄다.

### "선회각 제한이 피할 수 있게 만드는 핵심" 인가 — 아니었다

미사일이 한 프레임에 돌 수 있는 각도를 `MISSILE_TURN_RATE`(100°/s)로 제한한 것이
회피를 가능하게 만든 장치라는 설명인데, 직접 재 보면 **100°/s 는 무제한 선회와 결과가 같다.**

| 선회 제한 \ 표적 속도 | 40 px/s | 80 px/s | 120 px/s | 160 px/s |
|---|---|---|---|---|
| 무제한 | 0.0 | 7.2 | 72.7 | 84.4 |
| 100°/s (기본값) | 1.0 | 7.2 | 72.7 | 84.4 |
| 30°/s | 16.8 | 57.4 | 77.4 | 86.2 |

(미사일 90 px/s 일 때 최근접 거리 px. 히트박스 반지름이 약 21 px 이므로 그보다 크면 회피 성공)

회피를 가르는 건 **속도비**다. 표적이 미사일보다 빠르면 선회를 무제한 허용해도 빗나가고,
느리면 100°/s 로 제한해도 맞는다. 제한이 실제로 일하기 시작하는 건 30°/s 부근이다.
난이도를 조절하려면 `MISSILE_SPEED_*` 를 만지는 쪽이 직접적이다.

그래서 이 게임에는 **시간 상한**이 있다. 속도가 `90 + 4t` 로 올라 220 에서 멈추므로
**32.5 초부터는 속도로 피할 수 없다** — 그 뒤로는 미사일이 수명(7초)으로 사라지기를 기다리는 게임이다.

### 충돌 판정과 히트박스

원(미사일)과 사각형(드론)의 충돌은 **사각형 위 최근접점까지의 거리**로 본다.
`74` 가 사각형을 격자로 훑어 구한 최소 거리와 대조해 확인했다(판정 가능한 1,969 건 중 불일치 0).
`HITBOX_SCALE = 0.7` 은 한 변 70% 지만 **면적으로는 49%** 다 — 박스에 섞인 배경을 보정하는
값이지만 절반으로 줄인다는 사실은 알고 쓰는 편이 낫다(34강의 채움 비율과 같은 이야기).

### ⚠️ `drone.open(None)` 은 자동 탐색이 아니다

LED 연동 코드가 `DRONE_PORT = None` 을 그대로 `open()` 에 넘기는데, 라이브러리의 기본값은
**문자열 `"None"`** 이라 파이썬 `None` 은 자동 탐색 분기를 타지 않는다.
예외도 나지 않고 `False` 만 돌아오므로, 반환값을 안 보면 **연결되지 않은 채
"드론 LED 연결 OK" 가 찍힌다**(직접 호출해 확인: `open(None) -> False`, 이후 LED 명령은 조용히 `None`).
→ `drone_util.find_port()` 로 포트를 찾고 **반환값을 확인**한다.

### 그 밖에 고친 것

- 추적 놓침 판정 — MIL 폴백 환경에서는 `success` 만으로 영영 안 걸린다(35강) → 히스토그램 가드
- `make_surface(swapaxes)` → `image.frombuffer` (28강에서 4.72 ms vs 0.12 ms)
- SAM 마스크는 여러 후보 중 **클릭 점을 포함하는 것**을 고르고 해상도를 맞춘다(34강)
- 카메라가 없어도 돌도록 `SOURCE = "demo"` — 합성 드론이 떠다니는 화면에서 게임이 그대로 돈다

> ⚠️ 이 과제에서 **드론은 조종기로 직접 조종한다** — 코드는 비행 명령을 보내지 않는다
> (`USE_DRONE_LED = True` 일 때 LED 만 보낸다). 카메라 앞 1.5~2 m, 주변을 비우고 낮게 띄운다.
> 배경이 단순하고 밝을수록 추적이 붙는다.
...
| `findContours` 가 마스크를 못 읽음 | bool 배열을 그대로 넘겼다 | `astype(np.uint8) * 255` |
| 점처럼 작은 박스가 생김 | 노이즈 조각이 가장 큰 윤곽선이 됐다 | `MIN_AREA` 로 거른다 |
| bbox 가 화면과 어긋남 | 마스크 해상도가 프레임과 다르다 | `cv.resize(..., INTER_NEAREST)` 로 맞춘다 |
| 박스 중심으로 돌렸는데 물체가 가운데 안 옴 | 비스듬한 물체는 박스의 대부분이 배경(실측 14%) | 무게중심도 함께 본다 |
| `AttributeError: TrackerCSRT_create` | 일반 `opencv-python` 에는 CSRT 가 없다 (contrib 전용) | contrib 로 교체하거나 MIL 로 대체 (`create_tracker()`) |
| 대비책 `cv2.legacy...` 도 `AttributeError` | `legacy` 모듈 자체가 contrib 쪽 | 대비책이 아니다 — 있는 추적기를 골라 쓴다 |
| `TrackerVit/Nano/DaSiamRPN_create()` 가 에러 | 별도 onnx·caffe 가중치 파일이 필요 | 파일을 내려받거나 MIL 사용 |
| 물체를 놓쳤는데 박스가 그대로 남음 | **MIL** 은 놓친 뒤에도 `success=True` (실측 0/119 만 False, CSRT·KCF 는 69회 False) | 색 히스토그램 유사도로 별도 판정 |
| 클릭하자마자 박스가 사라짐 | 몇 픽셀짜리 영역으로 추적기를 초기화 | `MIN_BOX` 미만이면 init 하지 않는다 |
| 분할이 실패했는데 옛 박스가 되살아남 | 클릭 시 이전 tracker 를 해제하지 않았다 | 클릭 즉시 tracker/box 를 버린다 |
| 30 FPS 로 설정했는데 16 fps | `MIL.update()` 자체가 60 ms 대 (CSRT 48ms, KCF 7ms) | 정상. 추적기를 바꾸거나 해상도를 낮춘다 |
| 느린 PC 에서 점수가 더 잘 나옴 | 점수는 `delta_tick`, 물리는 `min(dt, 0.1)` — 8 fps 에서 세계만 80% 로 느려짐 | 점수도 같은 `dt` 로 누적 |
| 미사일이 절대 안 빗나감 / 너무 쉬움 | 선회각 제한(100°/s)은 사실상 무제한과 같다 | `MISSILE_SPEED_*` 로 조절. 선회는 30°/s 부근부터 효과 |
| 32초쯤부터 무조건 죽음 | 미사일 속도 상한 220 px/s = 드론이 낼 수 없는 화면속도 | 정상. `MISSILE_SPEED_MAX` 를 낮춘다 |
| 연결 안 됐는데 "드론 LED 연결 OK" | `open(None)` 은 자동 탐색이 아니다 (기본값이 문자열 `"None"`), 예외 없이 `False` | `find_port()` + 반환값 확인 |
| 게임 중 박스가 허공에 멈춘 채 점수만 오름 | MIL 이 추적 실패를 알리지 않음 | 히스토그램 가드로 `PAUSED` 전환 |
